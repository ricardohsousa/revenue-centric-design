...
**Principle.** ARR = size (not necessarily health). Burn rate = risk (time until death). CAC = efficiency (but obsessing over lowering it can worsen conversion with a less-qualified cohort). DAU/MAU = engagement (<20% = bad sign). LTV = product (does the customer see value?). Churn = fit (user doesn't understand or see value → cancels). NRR = real growth (>100% = good). Gross margin = model (<70% in SaaS = problem, except AI wrappers). Runway = survival (<12 months = danger). One metric is like a blood exam — one bad is critical, but all good is real health.
...
## The funnel math nobody does: churn forces you to replace the base forever
**Principle.** To sell to 2,500 people/month you must source that audience somewhere — building the product is the easy part; distributing is the game. With a 5% LP conversion you need 50,000 visitors; from ads at 3% CTR that's 1.6M impressions. Then the part few compute: at 20% monthly churn, average customer lifetime is 5 months — you replace the whole base in 5 months. Keeping 2,500 customers means replacing 500/month just to stand still: 500 conversions = 10,000 visitors = 333,000 impressions, every month, forever. (20% churn on low ticket is optimistic — many in the bubble run 40–50%. The bucket never fills.)
**Apply when.** Planning growth targets without a churn line in the model.
